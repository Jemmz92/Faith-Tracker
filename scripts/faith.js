...
Hooks.on("renderActorSheet", (app, html, data) => {
  ...
  // --- Insert Faith Panel below Class Section ---
  if (!html.find(".faith-panel").length) {
    const classSection = html.find(".character-info"); // Adjust selector if your sheet differs
    const faithPanel = $(`
...
Hooks.on("renderTokenHUD", (hud, html, token) => {
  ...
  faithButton.click(() => {
    ...
    if (sheetHtml) updateFaithDisplay(actor, sheetHtml);
  });
...
// --- Update Sheet Header and Panel Display ---
